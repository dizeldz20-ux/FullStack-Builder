...
metadata:
  hermes:
    tags: [hebrew, voice, ivr, tel-aviv, stt, tts]
    related_skills: [voice-agent-stack-research, phone-integration-twilio-vapi, hebrew-nlp-toolkit]
    changelog:
      - 1.0.3 (2026-06-26): Added troubleshooting entries plus reference update. Groq llama-4-scout as best Hebrew LLM (227-427ms, 18x faster than MiniMax). FreeSWITCH plus mod_audio_stream as the correct architecture when customer has existing PBX (do not replace with Vapi/LiveKit). System prompt for handling Deepgram Nova-3 mixed Latin+Hebrew output.
      - 1.0.2 (2026-06-26): TTS corrected to eleven_v3 plus Ruby's production voice settings (stability=0.45, similarity_boost=0.8, style=0.25, speed=0.94, output_format=mp3_44100_192) — vendor defaults produce audio that 'doesn't sound like Ruby at all' per the user. Reference file updated with full reproduction recipe.
      - 1.0.1 (2026-06-26): Added references/hebrew-voice-poc-recipe-2026-06-26.md with validated end-to-end POC recipe and gotchas (Deepgram whisper for Hebrew Unicode, MiniMax thinking:disabled, ElevenLabs Liam for Hebrew TTS).
...
**CRITICAL gotcha for Deepgram (validated 2026-06-26):** If you use Deepgram instead of Whisper/Google/Azure, only `model=whisper` returns Hebrew Unicode. Nova-2 doesn't support `he`. Nova-3 returns Latin transliteration, not Hebrew. See `references/hebrew-voice-poc-recipe-2026-06-26.md` for the full gotcha list.
...
def synthesize_hebrew(text: str, output_path: str, voice_gender: str = "female") -> None:
    ...
    response = client.synthesize_speech(
        input_text=input_text, voice=voice, audio_config=audio_config
    )
...
**CRITICAL gotcha for ElevenLabs (validated 2026-06-26, CORRECTED 2026-06-26 v1.0.2):** The default voice `Adam` (`pNInz6obpgDQGcFmaJgB`) does NOT speak Hebrew — produces garbled audio. For Hebrew TTS, use `Liam` (`TX3LPaxmHKxFdv7VOQHJ`). 

**However, the production code at `[vault-workspace]/[your-voice-product]-app/backend/src/services/elevenlabsTts.ts` does NOT use `eleven_multilingual_v2` (which this skill previously recommended). It uses `model_id=eleven_v3` with these voice settings:**
```typescript
RUBY_TTS_VOICE_SETTINGS = {
  stability: 0.45,           // not 0.5
  similarity_boost: 0.8,     // not 0.75
  style: 0.25,               // not 0
  use_speaker_boost: true,
  speed: 0.94,               // not 1.0
}
RUBY_TTS_OUTPUT_FORMAT = 'mp3_44100_192'  // not mp3 default
```

When the user reviewed the POC output produced with `eleven_multilingual_v2` he said "**היא לא נשמעת בכלל כמו רובי**" (it doesn't sound like Ruby at all). After switching to `eleven_v3` + Ruby's exact voice settings, the output matched the existing Ruby voice. **For any voice agent that should sound like an existing agent, copy the production TTS config verbatim — do not assume vendor defaults match.** See `references/hebrew-voice-poc-recipe-2026-06-26.md` for the full reproduction recipe with Ruby's settings.
...
- `references/sip-freeswitch-pbx.md` -- When the customer has their own PBX (FreeSWITCH/Asterisk/3CX) on their own server and only accepts SIP trunk as the integration boundary. Mandatory read before scoping any Israeli SMB voice bot engagement.
- `references/hebrew-voice-poc-recipe-2026-06-26.md` — Verified end-to-end POC recipe for Hebrew voice agents (Deepgram + Groq + ElevenLabs), with all the gotchas that bit us during the iPracticom engagement: Nova-2 doesn't support he, Nova-3 returns mixed Latin+Hebrew script (not clean), Scribe doesn't support he, Adam voice can't speak Hebrew, audio format traps. Mandatory read before claiming a Hebrew voice POC is "working". Updated v1.0.3 with: Groq llama-4-scout as default LLM (227-427ms, best Hebrew), system prompt for mixed-script STT output, FreeSWITCH + mod_audio_stream as the right architecture when customer has existing PBX.
...
- **Deepgram Nova models do NOT return Hebrew Unicode reliably** — use `model=whisper` instead (validated 2026-06-26).
- **ElevenLabs Adam voice does NOT speak Hebrew** — use Liam with multilingual_v2 (validated 2026-06-26).
- **ElevenLabs Scribe v1 does NOT support Hebrew STT** — produces noise/garbage (validated 2026-06-26).
- **MiniMax M2.7 defaults to reasoning mode** — always set `thinking: {type: "disabled"}` to avoid empty replies (validated 2026-06-26).
...
Cause: Using Standard-tier voices instead of Neural/Wavenet voices, OR using a non-Hebrew-capable voice like ElevenLabs Adam.
Solution: Switch to neural voices: Google Wavenet (he-IL-Wavenet-A/B) or Azure Neural (he-IL-HilaNeural). For ElevenLabs, use Liam voice (`TX3LPaxmHKxFdv7VOQHJ`) with `model_id=eleven_v3` (NOT multilingual_v2) and Ruby's production voice settings: `stability=0.45, similarity_boost=0.8, style=0.25, use_speaker_boost=true, speed=0.94, output_format=mp3_44100_192`. Adam does NOT speak Hebrew. Amazon Polly Hebrew only offers a standard voice (Avri, male) with no neural option. Neural voices are more expensive but significantly more natural. **For any voice agent that should sound like an existing agent, copy that agent's production TTS config verbatim** — do not assume vendor defaults match.
...
### Error: "MiniMax LLM returns empty reply"
Cause: M2.7 defaults to reasoning mode and consumes all max_tokens on internal thinking.
Solution: Set `"thinking": {"type": "disabled"}` in every MiniMax API call. Without this, the model "thinks" forever and returns nothing useful.

### Error: "ElevenLabs TTS produces audio that STT can't decode"
Cause: Using the default Adam voice which doesn't speak Hebrew — produces garbled audio that STT rejects as "speaking foreign language".
Solution: Switch to Liam voice (`TX3LPaxmHKxFdv7VOQHJ`) with `model_id=eleven_multilingual_v2`. Adam is English-native and cannot render Hebrew correctly.

### Error: "Deepgram STT returns Latin characters for Hebrew audio"
Cause: Nova-2 doesn't support `language=he`. Nova-3 returns Latin transliteration instead of Hebrew Unicode.
Solution: Use `model=whisper` (not Nova-2/Nova-3). Whisper returns actual Hebrew text with confidence ~0.89 on clean Hebrew audio.

### Error: "Hebrew voice POC is 10+ seconds end-to-end, voice feels robotic"
Cause: Using MiniMax M2.7 (or any reasoning LLM) without disabling reasoning, AND using ElevenLabs vendor-default voice settings.
Solution: Switch LLM to Groq `meta-llama/llama-4-scout-17b-16e-instruct` (227-427ms Hebrew, OpenAI-compatible endpoint, GROQ_API_KEY in `/root/.hermes/.env`). Copy the production TTS voice settings verbatim from the existing agent's source code (Liam + eleven_v3 + Ruby's exact stability/similarity/style/speed/output_format values) — never assume vendor defaults match. See v1.0.3 update in `references/hebrew-voice-poc-recipe-2026-06-26.md` for the full reasoning and a 5-model benchmark table.

### Error: "Should I use Vapi / LiveKit / a SaaS voice agent?"
Cause: Customer has an existing FreeSWITCH (or Asterisk/3CX) PBX and you're trying to figure out the architecture.
Solution: Do NOT replace the PBX. Build the AI agent as a Python service connected via FreeSWITCH's `mod_audio_stream` (already installed on most FreeSWITCH boxes). The PBX handles IVR, queue, CDR, billing, recording, DID routing, human fallback — the AI is just one "extension" in the dialplan. LiveKit SIP (431⭐) and Vapi are NOT PBX replacements — they handle media but not the operational features a real call center needs. See v1.0.3 update for the architecture diagram.