...
## Setup (one-time, approval-gated)

Do not run setup automatically. Prefer `npx hyperframes ...` for normal use. Ask before any `sudo`, package-manager install, global npm install, or setup script, because setup may install FFmpeg/system packages, `hyperframes` globally, and browser binaries. In non-interactive contexts, skip setup rather than guessing approval.
